% FFT-Subplot basierend auf vorhandenen Zeit-Subplots hinzuf�gen
% 
% Berechnet wird die FFT eines plots im Zeitbereich
% Die Zeitbereichs-Plots werden �ber die Handle-Matrix srcdata_hdl gelesen.
% Abtastrate und y-Daten werden damit bestimmt.
% Wenn srcdata_hdl ein N-D-Array ist, werden D FFT Verl�ufe berechnet und
% in die zugeh�rigen Koordinatensysteme (axes) in ax_f eingezeichnet
...
function plot_fft(ax_t, ax_f, srcdata_hdl)
    ...
    % anonyme Funktion erzeugen, damit zus�tzliche Argumente an
    % Callback-Funktion �bergeben werden k�nnen
    tmpfcn = @(obj,evd) (mypostcallback(obj,evd,ax_t, ax_f, srcdata_hdl));
    % Callback-Funktion f�r zoomen erzeugen
    fignumber = get(ax_t(1), 'Parent');
    h=zoom(fignumber);
    set(h,'ActionPostCallback',tmpfcn);
    % Callback-Funktion f�r Verschieben erzeugen
    h=pan(fignumber);
    set(h,'ActionPostCallback',tmpfcn); 
    ...
    for ii = 1:size(srcdata_hdl,1)
        ...
        for srchdl=srcdata_hdl(ii,:)
            if ~ishandle(srchdl) || srchdl==0
                continue; % Linie wurde wohl gel�scht
            end
            ...
            % Linien-Stil von Ursprungsdaten �bernehmen
